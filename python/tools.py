# -*- coding: utf-8 -*-
from primesense import openni2 as OP #https://pypi.org/project/primesense/
import numpy as np
from PyQt5 import QtWidgets, QtGui, QtCore
import cv2 #pip install opencv_python-3.4.5+contrib-cp36-cp36m-win_amd64.whl

def getFrames(file_name,splash):
    #получение данных фреймов из файла oni в массивы   
    OP.initialize('C:\\Program Files\\OpenNI2\\Redist\\') #путь к dll OpenNI2
    frColor = []
    frDepth = []    
    file = OP.Device(file_name)
    file.set_depth_color_sync_enabled
    colorStream = OP.VideoStream(file,OP.SENSOR_COLOR)#цветной поток
    depthStream = OP.VideoStream(file, OP.SENSOR_DEPTH)#глубина поток
    colorStream.start()
    depthStream.start()
